...
data "template_cloudinit_config" "k3s_server" {
  ...
  part {
    ...
    content = templatefile("${path.module}/files/k3s-install.sh", {
      ...
      k3s_cli_args = join(" ",
        concat(
          ["server --cluster-init --node-taint CriticalAddonsOnly=true:NoExecute --write-kubeconfig-mode 644 --node-label unravel.node.kubernetes.io/role=master --with-node-id"],
          [" --token ${local.k3s_cluster_secret} "],
  ...
  part {
    ...
    content = templatefile("${path.module}/files/k3s-install.sh", {
      ...
      k3s_cli_args = join(" ", concat(
        ["agent --node-label unravel.node.kubernetes.io/role=agent --with-node-id"],
        [" --token ${local.k3s_cluster_secret}"],
